...
resource "kubernetes_manifest" "veto_cluster" {
  manifest = {
    ...
    kind       = "VetoCluster"
    metadata   = { name = "veto", namespace = kubernetes_namespace.veto.metadata[0].name }
    spec = {
      licenseRef = { secretName = "veto-veto-operator-license", key = "license" }
      server     = { replicas = 2, image = "ghcr.io/plawio/veto-server@${var.server_digest}", storageDriver = var.storage_driver }
      dashboard  = { replicas = 1, image = "ghcr.io/plawio/veto-dashboard@${var.dashboard_digest}" }
