# Agent setup for `veto`

This is a deepsec scanning workspace. Project `veto` was just registered
(target: `..`). Setup is incomplete — `data/veto/INFO.md`
...
2. **Fill in `data/veto/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id veto
pnpm deepsec process --project-id veto
```
